...
async def cmd_show_collection(
    ...
    text_list = await get_text_list(state, algolia_client, "in_collection")
    if not text_list:
        await message.answer(_[language_code].COLLECTION_IS_EMPTY)
        return

    await message.answer(_[language_code].THIS_IS_COLLECTION)
    for chunk in get_chunks(text_list, 30):
        await message.answer("\n".join(chunk), disable_web_page_preview=True)


router.message.register(cmd_show_collection, flt.Command("collection"))
